...
- **Persisted provider health history — shipped 2026-07-19.** New `provider_health_checks` table (provider, healthy, latency_ms, checked_at + indexes on `(provider, checked_at DESC)` and `checked_at`) in `migrate.ts`. `HealthMonitor` writes fire-and-forget to `createProviderHealthHistoryStore()` alongside the existing in-memory `HealthStore` update — routing still reads the in-memory store synchronously, DB writes never block the hot path, failures only log. Opportunistic retention prune (>365 days, at most hourly, no rollups yet — raw history is the compounding asset). `getProviderHealthHistory(days)` returns per-provider uptime %, avg latency, p50/p95 over a 1–90 day window; exposed at `GET /v1/status/history?days=30` (default 30, max 90) and rendered as the Uptime record on `StatusPage.tsx` (refreshes every 30s). Unlike `/v1/status`'s existing "reliability" number (traffic-derived from usage events), this is poll-derived, so providers with no recent customer traffic still show real uptime data — this is the dataset the "uptime as a moat" / landing-page trust signal work depends on. **Follow-ups, not built:** weighting routing decisions by this historical data (data model is ready for it), daily rollups before prune if raw row volume becomes an issue, recording missed poll windows when the API itself is down.
